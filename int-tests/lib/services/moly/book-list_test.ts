...
const mockFantasyList = `
  <html>
    <div class="book_list">
      <div class="book_atom">
        <a data-id="fantasyBook" href="/konyvek/fantasyBook">Fantasy Book</a>
      </div>
    </div>
  </html>
`;
...
const mockSciFiList = `
  <html>
    <div class="book_list">
      <div class="book_atom">
        <a data-id="sciFiBook" href="/konyvek/sciFiBook">Sci-Fi Book</a>
      </div>
    </div>
  </html>
`;

describe("moly book list service", () => {
  beforeAll(async () => {
    await setup();
  });
  ...
  describe("createOrUpdateBooksFromMoly with a genre", () => {
    it("creates books from list and pending shelf", async () => {
      ...
      try {
        await createOrUpdateBooksFromMoly(2024, Genre.Fantasy);
    ...
    it("throws error when booklist does not exist", async () => {
      ...
      try {
        await assertRejects(
          async () => await createOrUpdateBooksFromMoly(2024, Genre.Fantasy),
          EntityNotFoundException,
        );
      } finally {
        axiosGetMock.restore();
  ...
  describe("createOrUpdateBooksFromMoly without a genre", () => {
    it("creates books for all book lists of the year", async () => {
      const pool = await getOrCreateDatabasePool();
      await createBookList(pool, {
        year: 2024,
        genre: Genre.Fantasy,
      ...
      const axiosGetMock = stub(
        ...
        (url: string) => {
          let data = mockBookHtml;
          if (url.includes("/lista/fantasy")) {
            data = mockFantasyList;
          }
          if (url.includes("/lista/sci-fi")) {
            data = mockSciFiList;
          }
          return Promise.resolve({
            data,
      ...
      try {
        await createOrUpdateBooksFromMoly(2024);

        const fantasyBook = await getBookByMolyId(pool, "fantasyBook");
        const sciFiBook = await getBookByMolyId(pool, "sciFiBook");
        assertExists(fantasyBook);
        assertExists(sciFiBook);
        assertEquals(fantasyBook?.year, 2024);
        assertEquals(fantasyBook?.genre, Genre.Fantasy);
        assertEquals(sciFiBook?.year, 2024);
        assertEquals(sciFiBook?.genre, Genre.SciFi);
      } finally {
        axiosGetMock.restore();
      }
    });

    it("throws error when there are no book lists for the year", async () => {
      await assertRejects(
        async () => await createOrUpdateBooksFromMoly(2024),
        EntityNotFoundException,
      );
    });
  });
